...
extern const unsigned char Char_HighSet[20][12];
...
extern const unsigned char Char_HighSet_Conductance_ED[15];
...
const unsigned char Char_ADD4[16]={"是否恢复出厂设置"};
...
void Highset_Force_Stop_GetWater(u8 now )
{
  ...
  for(i=0;i<8;i++)
  {
    Display_Buf[0][i]   =    Char_HighSet[15][i]; 
  }  
  Display_Buf[0][8]   =  ':';
  Display_RefreshAll();
   while(1)
...
extern void DS1302_10toData(void);
extern void BurstWrite1302(u8 *ptr);
/*******************************************************************************
* 函 数 名: void Restore_Factory_Setting(u8 now);
* 功    能: 恢复出厂设置
* 说    明: 
*******************************************************************************/
void Restore_Factory_Setting(u8 now)
{
	  u8   rl=1,rl1=0;
  u8  i;
  DisplayBuf_Clr();
						...
						{							
							...
							case KEY_ENTER:
														/*设置参数退出*/
															...
															{
																//设置时间
																Ds1302DataBuf10[0]=19;
																Ds1302DataBuf10[1]=1;
																Ds1302DataBuf10[2]=21;
																Ds1302DataBuf10[3]=12;
																Ds1302DataBuf10[4]=0;
																Ds1302DataBuf10[5]=0;
																Ds1302DataBuf10[6]=1; 
																DS1302_10toData();
																...
																Clean_GetWater_Data();
																//3.设置流量都为 1200ml/min
																AdminPra.FlowEDISet = 1200;
																AdminPra.FlowROSet = 1200;
																AdminPra.FlowUPSet = 1200;
																//4.校准系数设置为 0
																AdminPra.UPpra = 0;
																AdminPra.ROpra = 0;
																AdminPra.EDIpra = 0;
																AdminPra.SRpra = 0;
																//5.UP门限   18M/Ω
																AdminPra.UPthres = 1800;
																//6.UP循环门限 18M/Ω
																AdminPra.XHthres = 1800;
																//9.UP循环时间 20S
																AdminPra.XH_Timer = 2000;
																//10.RO量程    2000
																AdminPra.ROrange = 2;
																//11.EDI量程   2000
																AdminPra.EDIrange = 2;
																//7.RO门限    量程的5%
																AdminPra.ROthres = 100;
																//8.EDI门限   量程的5%
																AdminPra.EDIthres = 1800;
																//选择版本A
																AdminPra.Soft_Version = 0;
																Seve_Admindata();//保存所有参数
					...
					case KEY_ENTER:
								 switch(MenuIndex)
								{
									...
									case 8:Highset_Conductance_source_ADJ_Set(CursorIndex);//SC电导校准
													DisplayBuf_Clr();break;
									case 9:Highset_Conductance_EDI_ADJ_Set(CursorIndex);//EDI电阻校准
													DisplayBuf_Clr();break;
									...
									case 12:Highset_WaterQuality_RO(CursorIndex);		     //RO电导上限		
													DisplayBuf_Clr();break;
									case 13:Highset_WaterQuality_ED(CursorIndex);		     //EDI电阻下限
													DisplayBuf_Clr();break;
									...
									case 16:Highset_RO_Conductance_Rang(CursorIndex);			//RO电导量程
													DisplayBuf_Clr();break;
									case 17:Highset_WaterQuality_XH(CursorIndex);			       //循环上限电阻
									  			DisplayBuf_Clr();break;
									case 18:Highset_WaterQuality_XH_TM(CursorIndex);			   //循环时间
													DisplayBuf_Clr();break;
									case 19:Restore_Factory_Setting(CursorIndex);			        //恢复出厂设置
													DisplayBuf_Clr();break;
								}
								break;
