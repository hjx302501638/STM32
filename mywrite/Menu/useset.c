...
{
  u8 i;
	u8 temp = Machine_State;
	key_event_lesure();//退出忙碌标记
	if(Machine_State == CX_State) //正在冲洗状态就不能进入冲洗界面
	{
		VoiceOut(VOICE_WASH);
		return ;
	}
	Machine_State = CX_State;
  ...
  Display_RefreshAll();
		...
		#endif 	
				if(Machine_State != CX_State){Machine_State = temp;OppositeColor(2,now,8);return ;}//冲洗完成自动跳出
...
extern void Check_AD_Device(void);
      ...
      Display_RefreshAll();
		 ...
		 if(key.KEY_Falg)
		{
			key_event_buys();//进入忙碌标记
					switch(key.KEY_Val)
					{
						case Sleep_TM:Clr_Pictrue(); return 0;	//睡眠返回等待界面	
						case KEY_OFF_ON:break;
						case KEY_UP:
							if(rl==1)
							{
			...
			if(flag ==1)
			{
				...
				/*显示取水信息*/
					...
					else if(temp_buff[0] == EDI_Data)//显示 EDI水信息
					{
						...
						{
							for(i=0;i<5;i++)
							Display_Buf[3][i+10] = Char_Resistivity[i];//显示单位
							DisplayFloat1(3,3,temp_buff[10]);//显示参数
						}else {for(i=0;i<12;i++)Display_Buf[3][i+4] = Char_SensorBreak_SC[i];}//提示为选购模块
						Display_Buf[1][13]='M';Display_Buf[1][14]='L';
					}
