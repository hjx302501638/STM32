...
extern const unsigned char Char_GetWater[4][12];
...
void  Save_EDI_GETWarte(u16  ML)
{
	...
	{
	Writer_GetWater_Data(EDI_Data,temp,(u16)(quantify*100),0);
...
{
  u8 i;
  float quantify;u16 GET_COUNT=0;
	if(GET_Warte_Judeg())return ;//进行是否可以取水判断
	quantify = GET_ADVAL.GetEDI_AD();
	
	/****************禁止取水**********************/
	if((quantify*100)>AdminPra.EDIthres)//超出设定门限
	{
		if(AdminPra.Material_falg)//禁止取水
		{
			//输出语音
  ...
  {  
    ...
    if(GET_ADVAL.State2)//如果存水质检测模块
    {
      ...
      for(i=0;i<5;i++)
      {
        Display_Buf[1][i+9]   =    Char_Resistivity[i];//显示单位
      } 
    }
    else
    {
...
{
  u8 i;
  u8 MenuIndex,MAXMenu;
  u8 CursorIndex;
  u8 DisplayIndex,MAXDisplay;
  u8 CursorIndex_per;
					...
					case KEY_ENTER:
								 switch(MenuIndex)
								{
									...
									case 1: GetUP(CursorIndex);//取UP水
													DisplayBuf_Clr();break;
									case 4:GetEDI(CursorIndex);//取EDI水									
													DisplayBuf_Clr();break;
									case 2:GetRO_Quantify(CursorIndex);//定量取RO水			
												  DisplayBuf_Clr();break;
									case 3:GetUP_Quantify(CursorIndex);//定量取UP水			
												  DisplayBuf_Clr();break;
									 ...
									 if(CursorIndex == 0)
										{
											...
											if((DisplayIndex ==0 )&&(CursorIndex == 0))
												{
													CursorIndex = 3;  //回到最底层	
													DisplayIndex=MAXDisplay;	
													MenuIndex		= MAXMenu;//选择了第3条目录											
												}else DisplayIndex--; 
										}
										else 
										{
